...
ID_to_latitude_level_3 = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\shape_info\mit-grid-level-3.csv', index_col='id')
latitudes_level_3 = pd.DataFrame(index = correct_index_level_3)
latitudes_level_3['Lat'] = ID_to_latitude_level_3.iloc[:, -2]
optimal_tilt_level_3 = latitudes_level_3 * 0.76 + 3.1

#putting latitudes into high res in a column
correct_shape_df_level_3['values'] = latitudes_level_3['Lat']
...
latitudes_level_3_high_res = np.repeat(correct_shape_np_final, lcm_width / solar_level_3_width, axis=1)
latitudes_level_3_high_res = np.repeat(latitudes_level_3_high_res, lcm_height / solar_level_3_height, axis=0)
latitudes = np.reshape(latitudes_level_3_high_res, (lcm_height * lcm_width), order='F')
...
for i in range(8783):
    ...
    I_horiz_level_3.columns = ['PAIRS polygon ID level 3', 'averaged_value']
    ...
    snow_SWE_level_2.columns = ['PAIRS polygon ID level 2', 'averaged_value']
    snow_SWE_level_2 = snow_SWE_level_2.set_index('PAIRS polygon ID level 2', drop = True)

    #unit conversions
    temp_C_level_3 = temp_K_level_3 - K_to_C
    I_horiz_Wh_level_3 = I_horiz_level_3 * Wh_per_J
    snow_SWE_mm_level_2 = snow_SWE_level_2 * 1000

    #irradiance_conversion based on tilt
    declination_angle = 23.45 * math.sin(math.radians(360/365 * (284 + day[i])))
    elevation_angle = 90 - latitudes_level_3['Lat'] + declination_angle
    alpha_plus_beta = np.deg2rad(elevation_angle.to_frame()) + np.deg2rad(optimal_tilt_level_3)
    ...
    irradiance_level_3 = I_horiz_Wh_level_3['averaged_value'] * division_results['Lat']
    ...
    correct_shape_df_level_2['values'] = snow_SWE_mm_level_2['averaged_value']
    ...
    snow_SWE_mm_level_2_high_res = np.repeat(correct_shape_np_final, lcm_width / solar_level_2_width, axis=1)
    snow_SWE_mm_level_2_high_res = np.repeat(snow_SWE_mm_level_2_high_res, lcm_height / solar_level_2_height, axis=0)
    snow_SWE_mm = np.reshape(snow_SWE_mm_level_2_high_res, (lcm_height * lcm_width), order = 'F')

    correct_shape_df_level_3['values'] = irradiance_level_3.to_frame()
    correct_shape_np = correct_shape_df_level_3.to_numpy()
    correct_shape_np_final = np.reshape(correct_shape_np, (solar_level_3_height, solar_level_3_width), order='F')
    irradiance_level_3_high_res = np.repeat(correct_shape_np_final, lcm_width / solar_level_3_width, axis=1)
    irradiance_level_3_high_res = np.repeat(irradiance_level_3_high_res, lcm_height / solar_level_3_height, axis=0)
    irradiance = np.reshape(irradiance_level_3_high_res, (lcm_height * lcm_width), order = 'F')
...
tracking_solar_output.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\tracking_solar_output_level_3_and_snow_level_2.csv')