...
solar_power_output_level_2 = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\level_2\solar_power_output_sum_GWh_level_2.csv')
solar_power_output_level_2 = solar_power_output_level_2.drop(columns=solar_power_output_level_2.columns[0])

#recording dimensions
solar_level_1_width = solar_power_output_level_1.shape[1]
solar_level_1_height = solar_power_output_level_1.shape[0]

solar_level_2_width = solar_power_output_level_2.shape[1]
solar_level_2_height = solar_power_output_level_2.shape[0]

#finding LCM
#height
for i in range(1, solar_level_2_height + 1):
    if i * solar_level_1_height % solar_level_2_height == 0:
        lcm_height = i * solar_level_1_height
        break

#width
for j in range(1, solar_level_2_width):
    if j * solar_level_1_width % solar_level_2_width == 0:
        lcm_width = j * solar_level_1_width
        break

error_at_high_res = np.empty([lcm_height, lcm_width])

solar_power_output_level_2_np = solar_power_output_level_2.to_numpy()
solar_power_level_2_high_res = np.repeat(solar_power_output_level_2_np, lcm_width/solar_level_2_width, axis = 1)
solar_power_level_2_high_res = np.repeat(solar_power_level_2_high_res, lcm_height/solar_level_2_height, axis = 0)
...
error_at_high_res = (solar_power_level_2_high_res - solar_power_level_1_high_res) / solar_power_level_1_high_res * 100
#make a heatmap out of
plt.imshow(error_at_high_res, cmap='seismic') #'seismic_r'
plt.colorbar()
...
plt.title('Error distribution level 2')#, fontweight="bold")
