...
ax.set_ylabel('Solar power output with level 2 aggregation (W)', fontsize=12)
ax.set_xlim([0, 8783])
ax.set_ylim(0)
plt.show()
...
correct_shape_df = pd.DataFrame(index = range(384), columns = ['yearly_sum'])
correct_shape_df['yearly_sum'] = yearly_sum
correct_shape_np = correct_shape_df.to_numpy()
correct_shape_np_final = np.reshape(correct_shape_np, (12, 32), order = 'F')
...
plt.colorbar()
plt.title('Solar yearly sum output level 2 (GWh)')
plt.clim(65, 100)
plt.show()

correct_shape_np_final_GWh_df = pd.DataFrame(correct_shape_np_final_GWh)
correct_shape_np_final_GWh_df.to_csv('solar_power_output_sum_GWh_level_2.csv')
...
plt.title('Solar yearly sum output distribution level 2')#, fontweight="bold")

(mu, sigma) = norm.fit(yearly_sum/1000000000)
...
#change the shape to be the way it should be
std_shape_df = pd.DataFrame(index = range(384), columns = ['standard_deviation'])
std_shape_df['standard_deviation'] = std
std_shape_np = std_shape_df.to_numpy()
std_shape_np_final = np.reshape(std_shape_np, (12, 32), order = 'F')
...
#change the shape to be the way it should be
cov_shape_df = pd.DataFrame(index = range(384), columns = ['standard_deviation'])
cov_shape_df['standard_deviation'] = cov
cov_shape_np = cov_shape_df.to_numpy()
cov_shape_np_final = np.reshape(cov_shape_np, (12, 32), order = 'F')
