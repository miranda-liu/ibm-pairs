...
solar_data = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\level_2\tracking_solar_output_level_2.csv')
solar_data = solar_data.set_index('PAIRS polygon ID level 2', drop = True)
...
LCA.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\level_2\solar_LCA_column_level_2.csv')
...
LCA_shape_df = pd.DataFrame(index = range(384), columns = ['LCA'])
LCA_shape_df['LCA'] = LCA
LCA_shape_np = LCA_shape_df.to_numpy()
LCA_shape_np_final = np.reshape(LCA_shape_np, (12, 32), order = 'F')
...
LCA_shape_df_final.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\level_2\solar_LCA_level_2.csv')

#comparing SESAME to PAIRS results
boston_PAIRS = float(LCA.loc[351]) #for level 2
boston_SESAME = float(35.0)

seattle_PAIRS = float(LCA.loc[12])
seattle_SESAME = float(42)

miami_PAIRS = float(LCA.loc[299])
miami_SESAME = float(32)

tuscon_PAIRS = float(LCA.loc[92])
...
boston_cf_difference = abs(float(cf.loc[351]) - 15.6) / statistics.mean([cf.loc[351], 15.6]) * 100
boston_LCA_difference = abs(boston_PAIRS - boston_SESAME) / statistics.mean([boston_PAIRS, boston_SESAME]) * 100

seattle_cf_difference = abs(float(cf.loc[12]) - 12.69) / statistics.mean([cf.loc[12], 12.69]) * 100
seattle_LCA_difference = abs(seattle_PAIRS - seattle_SESAME) / statistics.mean([seattle_PAIRS, seattle_SESAME]) * 100

miami_cf_difference = abs(float(cf.loc[299]) - 16.72) / statistics.mean([cf.loc[299], 16.72]) * 100
miami_LCA_difference = abs(miami_PAIRS - miami_SESAME) / statistics.mean([miami_PAIRS, miami_SESAME]) * 100

tuscon_cf_difference = abs(float(cf.loc[92]) - 20.53) / statistics.mean([cf.loc[92], 20.53]) * 100
