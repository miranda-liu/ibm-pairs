...
conversion = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\level_2\conversion.csv')
correct_index = conversion['PAIRS polygon ID']
conversion = conversion.set_index('PAIRS polygon ID', drop=True)
sum = pd.DataFrame(float(0), index = range(384), columns = ['total'])

for index, row in conversion.iterrows():
    new_index = conversion.loc[index, 'level_2 value']
    sum.loc[new_index, 'total'] += conversion.loc[index, 'count']


wind_dataframe = pd.DataFrame(float(0), index = range(384), columns = ['averaged value', 'testing column'])
irradiance_dataframe = pd.DataFrame(float(0), index = range(384), columns = ['averaged value', 'testing column'])
temperature_dataframe = pd.DataFrame(float(0), index = range(384), columns = ['averaged value', 'testing column'])
snow_dataframe = pd.DataFrame(float(0), index = range(384), columns = ['averaged value', 'testing column'])
...
for i in range(8783):
    ...
    wind_dataframe.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_2//' + 'Global weather (ERA5)-wind value-' + current_timestamp + 'level_2.csv', index = True)
    irradiance_dataframe.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_2//' + 'Global weather (ERA5)-Solar radiation-' + current_timestamp + 'level_2.csv', index=True)
    temperature_dataframe.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_2//' + 'Global weather (ERA5)-Temperature-' + current_timestamp + 'level_2.csv', index = True)
    snow_dataframe.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_2//' + 'Global weather (ERA5)-Snow depth-' + current_timestamp + 'level_2.csv', index=True)
