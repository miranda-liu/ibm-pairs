...
testing = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_3\Global weather (ERA5)-wind value-01_01_2020T01_00_00level_3.csv')
testing.columns = ['PAIRS polygon ID level 3', 'averaged value']
correct_index = testing['PAIRS polygon ID level 3']
testing = testing.set_index('PAIRS polygon ID level 3', drop = True)
...
ID_to_latitude = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\shape_info\mit-grid-level-3.csv', index_col='id')
ID_to_latitude = ID_to_latitude.iloc[:50, :6]
...
for i in range(8783):
    ...
    v = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_3//' + 'Global weather (ERA5)-wind value-' + current_timestamp + 'level_3.csv') #m/s
    v.columns = ['PAIRS polygon ID level 3', 'averaged value']
    v = v.set_index('PAIRS polygon ID level 3', drop=True)
    I_horiz = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_3//' + 'Global weather (ERA5)-Solar radiation-' + current_timestamp + 'level_3.csv') #J/m^2
    I_horiz.columns = ['PAIRS polygon ID level 3', 'averaged value']
    I_horiz = I_horiz.set_index('PAIRS polygon ID level 3', drop=True)
    temp_K = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_3//' + 'Global weather (ERA5)-Temperature-' + current_timestamp + 'level_3.csv')  # K
    temp_K.columns = ['PAIRS polygon ID level 3', 'averaged value']
    temp_K = temp_K.set_index('PAIRS polygon ID level 3', drop=True)
    snow_SWE = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\Spatially_aggregated_level_3//' + 'Global weather (ERA5)-Snow depth-' + current_timestamp + 'level_3.csv')  # m water equivalent
    snow_SWE.columns = ['PAIRS polygon ID level 3', 'averaged value']
    snow_SWE = snow_SWE.set_index('PAIRS polygon ID level 3', drop=True)
...
tracking_solar_output.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\level_3\tracking_solar_output_level_3.csv')
print('done with level 3 calculations :)')