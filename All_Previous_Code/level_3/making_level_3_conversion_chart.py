...
level_2_coordinates = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\shape_info\mit-grid-level-3.csv', index_col='id')
level_2_coordinates = level_2_coordinates.iloc[:50, :6]

conversion = pd.DataFrame(index = correct_index, columns = ['level_3 value'])
...
for index, row in conversion.iterrows():
    ...
    for i in range(50):
        if longitude > level_2_coordinates.iloc[i, 3] and longitude <= level_2_coordinates.iloc[i, 1] and latitude > level_2_coordinates.iloc[i, 0] and latitude <= level_2_coordinates.iloc[i, 2]:
            conversion.loc[index, 'level_3 value'] = i
            continue

conversion['count'] = testing.iloc[:, 0]

conversion.to_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\level_3\conversion_level_3.csv')