...
solar_data = pd.read_csv(r'C:\Users\ChemeGrad2019\Desktop\IBM_PAIRS\New_data_format\tracking_solar_output_level_3_and_wind_level_2.csv', index_col = 0)
...
plt.colorbar()
plt.title('Solar yearly sum output level 3 and 2 wind (GWh)')
plt.clim(65, 100)
plt.show()

correct_shape_np_final_GWh_df = pd.DataFrame(correct_shape_np_final_GWh)
correct_shape_np_final_GWh_df.to_csv('solar_power_output_sum_GWh_level_3_level_2_wind.csv')
...
plt.title('Solar yearly sum output distribution level 3 and 2 wind')#, fontweight="bold")
...
print('hi amanda. combo done with wind level 2')