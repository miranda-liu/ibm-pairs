...
with open('/Users/mirandaliu/Documents/GitHub/ibm-pairs/Pipeline_B/random_wind_capacity_factors.csv') as file_obj:
    # skip header
    ...
    for row in cf_data:
        ...
        all_wind_rfs.append(rf_location_yearly)
...
def rf_3d_matrix_wind():
    ...
    rf_3dmatrix_wind = np.ndarray(shape=(hours,width,length), dtype=float, order='F')
    np.set_printoptions(suppress=True) # supresses scientific notation in np array

    temp_list = []
    for i in range(hours):
        for j in range(width * length):
            temp = all_wind_rfs[j][i]
            temp_list.append(temp)
    for i in range(hours):
        for k in range(width):
            for l in range(length):
                rf_3dmatrix_wind[i][k][l] = temp_list.pop(0)
          
    return rf_3dmatrix_wind
